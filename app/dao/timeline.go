...
func (r *timeline) GetHome(ctx context.Context, accountId int, only_media bool, max_id int, since_id int, limit int) (*object.TimeLine, error) {
	entity := new(object.TimeLine)
	query := `
		select statuses.*, 
		account.username "account.username", 
		...
		from statuses 
		join account on statuses.account_id = account.id
	` + `where statuses.account_id =` + strconv.Itoa(accountId)

	// Add conditions for max_id and since_id
	if max_id > 0 && since_id > 0 {
		query += " AND statuses.id <= " + strconv.Itoa(max_id) + " AND statuses.id >= " + strconv.Itoa(since_id)
	} else if max_id > 0 {
		query += " AND statuses.id <= " + strconv.Itoa(max_id)
	} else if since_id > 0 {
		query += " AND statuses.id >= " + strconv.Itoa(since_id)
	}
